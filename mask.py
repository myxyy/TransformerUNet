...
class CrossEncodeMask(nn.Module):
    def __init__(self, dim):
        ...
        self.mask = nn.Upsample((dim*2,dim))(torch.full((dim,dim), float('-inf')).tril(-1).view(1,1,dim,dim)).view(dim*2,dim).roll(-1,0).index_add(0,torch.tensor([dim-1]),torch.full((1,dim),float('-inf'))).transpose(1,0).cuda()
    def forward(self, x):
        return x + self.mask
